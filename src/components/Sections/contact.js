...
export const ContactSection = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        formBody.append("message", formData.message);       //construct form data

        try {
            const response = await fetch("https://formsubmit.co/ajax/ocam12@btinternet.com", {
                method: "POST",
                body: formBody,
                headers: { "Accept": "application/json" },
            });

            if (response.ok) {      //display alert depending on status of message
    ...
    return (
        <section id="contact">
            ...
            <div className="contact-information">
                <h3>Get In Touch!</h3>
                <address>
                    <p>✉️ Email: <span><a href="mailto:ocam12@btinternet.com" className="hover-mouse" tabIndex="0">ocam12@btinternet.com</a></span></p>
                </address>
            ...
            <form onSubmit={handleSubmit} className="contact-form">
                <h4>Or Send Me A Message Here!</h4>
                <div className="input-container">
                    <label htmlFor="name">Your Name</label>
                    <input id="name" className="hover-mouse" type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Your Name..." required aria-required="true"></input>
                </div>
                <div className="input-container">
                    <label htmlFor="email">Your Email</label>
                    <input id="email" className="hover-mouse" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Your Email..." required aria-required="true"></input>
                </div>
                <div className="input-container">
                    <label htmlFor="message">Your Message</label>
                    <textarea id="message" className="hover-mouse" rows={8} name="message" value={formData.message} onChange={handleChange} placeholder="Your message..." required aria-required="true"></textarea>
                </div>
                <button type="submit" className="submit-btn hover-mouse" tabIndex="0">Submit</button>
            </form>
            <Anchor className="next-page-link" fontSize={'1.4rem'} link={"#hero"} text={<>Back To Top <span className="arrow">↑</span></>}></Anchor>
        </section>
    )
}